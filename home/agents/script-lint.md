...
- **Setup/install/uninstall scripts** (`install.sh`, `setup.sh`, `uninstall.sh`, and variants) that export vars, or read them as a caller-settable override, must use `{PROJECT_NAME}_` as the prefix on those vars — never `INSTALL_`, `SETUP_`, or `UNINSTALL_`. `{PROJECT_NAME}` is the project/repo name uppercased. Flag any such var in these scripts using the script filename as prefix instead of the project name. Purely internal globals follow the same own-process exemption above.
