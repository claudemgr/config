...
- **Variables**:
  ...
  - **Never shadow system environment variables** — never assign directly to a name that is an existing system env var (`HOME`, `USER`, `LOGNAME`, `SHELL`, `PATH`, `PWD`, `OLDPWD`, `HOSTNAME`, `HOST`, `TERM`, `LANG`, `TZ`, `EDITOR`, `VISUAL`, `PAGER`, `UID`, `EUID`, `GID`, `SHLVL`, `PS1`–`PS4`, `OPTIND`, `OPTARG`, and all `BASH_*`/`ZSH_*`/`FISH_*` vars). Use the project-prefixed name and take the system var as a fallback: `MYSCRIPT_FQDN="${MYSCRIPT_FQDN:-$HOSTNAME}"` not `HOSTNAME="$MYSCRIPT_FQDN"`. System env vars may be read freely — never written. Exception: `IFS` may be changed temporarily — always save and restore, or scope to a subshell: `old_IFS="$IFS"; IFS=:; …; IFS="$old_IFS"` or `( IFS=:; … )`.
