...
- **Variables**:
  ...
  - **Never overwrite system environment variables** — system env vars (`HOME`, `USER`, `LOGNAME`, `SHELL`, `PATH`, `PWD`, `OLDPWD`, `HOSTNAME`, `HOST`, `TERM`, `LANG`, `TZ`, `EDITOR`, `VISUAL`, `PAGER`, `UID`, `EUID`, `GID`, `SHLVL`, `PS1`–`PS4`, `OPTIND`, `OPTARG`, and all `BASH_*`/`ZSH_*`/`FISH_*` vars) must never be assigned to — doing so clobbers values the shell and other tools depend on. Reading them is always fine and expected: use them freely as fallbacks, in conditions, anywhere (`RUN_USER="${SUDO_USER:-$USER}"`, `MYSCRIPT_FQDN="${MYSCRIPT_FQDN:-$HOSTNAME}"`). Exception: `IFS` may be temporarily changed — always save and restore, or scope to a subshell: `old_IFS="$IFS"; IFS=:; …; IFS="$old_IFS"` or `( IFS=:; … )`.
