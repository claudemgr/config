...
- **`cd` always uses absolute paths** — never `cd relative/path` or `cd ../foo`; always `cd /full/path/to/dir`. No exceptions.
- **External commands always use `\command`** — prefix every external command with `\` to bypass aliases and shell functions: `\curl`, `\git`, `\grep`, `\chmod`, `\mkdir`, `\cp`, `\mv`, `\rm`, etc. In fish use `command curl` (fish does not support `\` bypass). This ensures the real binary runs regardless of aliases or wrapper functions defined in the user's environment.
