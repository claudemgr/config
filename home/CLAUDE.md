...
- **Coverage and test output go to the OS temp dir, never the project tree** — write `-coverprofile=/tmp/coverage.out` inside the container (ephemeral with `--rm`), or mount a `${TMPDIR:-/tmp}/{project_org}/{project_name}-XXXXXX/` dir. The project directory is source code only. CI runner workspaces are ephemeral and exempt.
