...
- **Provider inference** — first infer `{provider_name}` from the directory name itself: `github` → github.com, `gitlab` → gitlab.com. When the directory name is ambiguous (`gitea`, `private`, `local`, or any name that doesn't map directly to a known host), confirm with `git -C {project_dir} remote get-url origin` and match the host in the URL instead of guessing from the path alone.
- **`~/Projects/private`** hosts this machine's own self-hosted git instance. Detect it by matching `git -C {project_dir} remote get-url origin`'s host against the `$GIT_PRIVATE_URL` env var (this machine's own private git host, e.g. `https://casjay.work`) when it's set; if the remote host matches, it's confirmed `private`. Fall back to the remote alone (host doesn't match any known provider, or `$GIT_PRIVATE_URL` is unset) — never hardcode the host, since it's env-configured per machine.
