...
- **Never shadow system environment variables** — never assign directly to `HOME`, `USER`, `LOGNAME`, `SHELL`, `PATH`, `PWD`, `OLDPWD`, `HOSTNAME`, `HOST`, `TERM`, `LANG`, `TZ`, `EDITOR`, `VISUAL`, `PAGER`, `UID`, `EUID`, `GID`, `SHLVL`, `IFS`, `PS1`–`PS4`, `OPTIND`, `OPTARG`, or any `BASH_*`/`ZSH_*`/`FISH_*` var. Use the script/project name as prefix (`{PROJECT_NAME}_VARNAME` — never generic `APP_`, `MY_`, etc.) and take the system var as a fallback: `MYSCRIPT_FQDN="${MYSCRIPT_FQDN:-$HOSTNAME}"` not `HOSTNAME="$MYSCRIPT_FQDN"`. Applies to all languages and scripts. Full list in `script_conventions.md`.
