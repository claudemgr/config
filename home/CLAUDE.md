...
- **Search before write** — before adding a value, enumerate every place it could already live (e.g. config files, env files, code constants, docs — not just the one you thought of first) and grep each one; only after all are checked and come up empty is create/append allowed; replace in place if found in any of them
