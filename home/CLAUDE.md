...
- **Coverage and test output never go to the project tree** — where they go depends on context: inside a `--rm` container use `/tmp/coverage.out` (the whole container filesystem is destroyed on exit, so full namespacing is unnecessary); on the host use `${TMPDIR:-/tmp}/{project_org}/{internal_name}-XXXXXX/coverage.out`; in CI runner workspaces the workspace is ephemeral and exempt. The project directory is source code only.
