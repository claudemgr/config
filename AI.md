...
| `test-lint-mark.sh` | PostToolUse Bash | Records, per session and project, that a test-gate or lint-gate command exited 0 (and wasn't interrupted) — pairs with `enforce-test-lint-gate.sh`. A confirmed upstream bug ([anthropics/claude-code#36310](https://github.com/anthropics/claude-code/issues/36310), open) means `PostToolUse` on `Bash` sometimes never spawns for a real Bash tool call, so the marker isn't written even though the command genuinely passed — `enforce-test-lint-gate.sh` then falsely reports the gate as unrun. This is not a bug in this hook's own matching/marker logic (verified correct); if the gate blocks right after a real passing `bash -n`/test/lint run, re-run the same command once more before assuming the fix is wrong |
