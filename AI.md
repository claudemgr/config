...
| `no-read-gitcommit.sh` | PreToolUse Read+Grep+Bash | Blocks reading the `gitcommit` script file (Read tool, Grep tool, and `cat`/`less`/`head`/etc. via Bash) — CLAUDE.md's Commit Workflow says it is "pre-approved and trusted" and must never be inspected, only invoked; no zone exception |
...
| `enforce-test-lint-gate.sh` | PreToolUse Bash | Blocks `gitcommit --dir <path> all` unless the test gate and lint gate actually ran and passed this session for that project. Checks two independent signals, either satisfies the gate: (1) the markers `test-lint-mark.sh`/`lint-agent-mark.sh` write, or (2) a direct scan of the PreToolUse payload's own `transcript_path` for a passing (`is_error` false, not `interrupted`) Bash `tool_use`/`tool_result` pair this session whose `cwd` matches the target project and whose command matches the test/lint pattern — a fallback for the upstream PostToolUse bug documented below, added because marker-only enforcement was blocking real passing runs across every project/session. Spec-collection projects (no Makefile/manifest/`*.sh`) substitute an AI.md/SPEC.md re-read this session instead, reusing `spec-guard.sh`'s marker; a template repo with neither AI.md nor SPEC.md at its root (`claudemgr/{go,rust,android,docker,mgr}`) satisfies this via `spec-guard-mark.sh`'s root-level `*.md` fallback instead |
| `test-lint-mark.sh` | PostToolUse Bash | Records, per session and project, that a test-gate or lint-gate command exited 0 (and wasn't interrupted) — pairs with `enforce-test-lint-gate.sh`. A confirmed upstream bug ([anthropics/claude-code#36310](https://github.com/anthropics/claude-code/issues/36310), open) means `PostToolUse` on `Bash` sometimes never spawns for a real Bash tool call, so the marker isn't written even though the command genuinely passed — `enforce-test-lint-gate.sh` then falsely reported the gate as unrun before it gained the `transcript_path` fallback above, which now covers this case without depending on `PostToolUse` firing at all. This is not a bug in this hook's own matching/marker logic (verified correct) |
| `lint-agent-mark.sh` | SubagentStop | Records the lint gate as satisfied when the `script-lint`/`go-lint`/`rust-lint` subagent finishes for this project and session with a clean report — the lint gate is actually run via the Agent tool (a subagent), never as a host CLI command, so `test-lint-mark.sh`'s PostToolUse-on-Bash detection can never see it fire. Parses the SubagentStop payload's `last_assistant_message` against the lint agents' own `: clean` / `: N issue(s) found` report contract; any outstanding-issues line anywhere in a multi-file report skips the marker. Writes the same marker `enforce-test-lint-gate.sh` checks |
